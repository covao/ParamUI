...
class paramui:
    ...
    - ParameterTable: List containing [ParameterVariable, ParameterLabel, InitialValue, Range]
        ...
        - Range: Widget type specification
            ...
            - Textbox with regex validation: a regex pattern string, e.g. '[A-Za-z ]{0,32}'
              The value is only committed when it fully matches the pattern.
              When the textbox loses focus with a non-matching value, it reverts
              to the last valid value instead.
    ...
    def __init__(self, parameter_table=[], show_ui=True):
        ...
        self.patterns = {}  # Map parameter variables to compiled regex patterns (textbox validation)
        self.last_valid_value = {}  # Map parameter variables to their last value that matched the pattern
        self.pattern_entries = {}  # Map parameter variables to (Entry widget, StringVar) for live color feedback
        self.pattern_vars = {}  # Map parameter variables to their textbox's StringVar
        ...
        if parameter_table and self.ShowUI and chibiui:
            ...
            # First pass: compute widget paths and register regex patterns
            # for every parameter before any widget is created, so that the
            # FocusOut validation (see _setup_pattern_validation) is already
            # wired up when the very first textbox gets rendered.
            for variable, label, initial_value, step in parameter_table:
                ...
                if (isinstance(initial_value, str) and isinstance(step, str) and step
                        and not (step.startswith('*.') or step == 'folder')):
                    self._register_pattern(variable, initial_value, step)

            # Bind pattern validation to textboxes so invalid values are
            # rejected when the user leaves the field (composition-based
            # instrumentation only; chibiui's class code is left untouched)
            ...
            for i, param in enumerate(parameter_table):
                ...
                widget_path = self.widget_paths[variable]
                ...
                elif isinstance(initial_value, str) and isinstance(step, str) and step:
                    # Text box widget with regex pattern validation
                    # (pattern already registered in the first pass above)
            ...
            # the navigation tree window to the left edge since it's unused
            if not any('/' in param[0] for param in parameter_table):
                self._minimize_nav_tree()
    ...
    def _init_parameters_only(self, parameter_table):
        ...
        for param in parameter_table:
            ...
            # Create nested structure for path-like variables
            self._create_nested_structure(variable, initial_value)
            # Register regex pattern (if any) for later validation in update_prm()
            if isinstance(step, str) and step and not (step.startswith('*.') or step == 'folder' or step == 'button'):
                self._register_pattern(variable, initial_value, step)
    ...
    def _register_pattern(self, variable, initial_value, step):
        """
        Compile and register a regex pattern for a textbox parameter.

        Args:
            variable (str): The parameter path (e.g. 'Name1', 'Options/Text1').
            initial_value (str): The initial value of the parameter.
            step (str): The regex pattern string (e.g. '[A-Za-z ]{0,32}').
        """
        try:
            self.patterns[variable] = re.compile(step)
            self.last_valid_value[variable] = initial_value
        except re.error as e:
            print(f"Warning: invalid regex pattern for '{variable}': {step} ({e})")

    def _setup_pattern_validation(self):
        """
        Attach live regex validation to textbox widgets that have a pattern.

        While typing, the entered text is checked against the pattern on
        every keystroke and the text color turns red when it doesn't match
        (green/default otherwise). When the field loses focus (or focus
        moves to another widget) with a non-matching value, the textbox is
        reverted to its last valid value.

        This does NOT modify the chibiui class itself: it wraps the
        `_create_textbox` method on the specific chibiui *instance* created
        for this paramui (composition/instrumentation), so the pattern is
        checked whenever a textbox is (re)created, and the resulting Entry
        widget is bound to validate its value as it's typed and on focus-out.
        """
        if not self.patterns or not self.ui or self.ui.nogui:
            return

        # Build a lookup from chibiui's internal widget key ("/path/Label")
        # back to the original parameter variable name.
        key_to_variable = {}
        for variable, widget_path in self.widget_paths.items():
            if variable in self.patterns:
                key = widget_path if widget_path.startswith('/') else '/' + widget_path
                key_to_variable[key] = variable

        ui = self.ui
        original_create_textbox = ui._create_textbox

        def patched_create_textbox(label, value):
            original_create_textbox(label, value)
            try:
                full_key = ui._get_full_key(ui.current_path, label)
                variable = key_to_variable.get(full_key)
                if variable is None:
                    return
                frame = ui._get_current_frame()
                ...
                self.pattern_entries[variable] = entry
                # Live validation: color the text red while it doesn't match
                entry_var = ui.value.get(full_key)
                self.pattern_vars[variable] = entry_var
                self._apply_pattern_color(variable, entry, text=entry_var.get() if entry_var is not None else None)
                if entry_var is not None:
                    entry_var.trace_add('write', lambda *a, v=variable: self._on_pattern_text_changed(v))

                # Revert to the last valid value when the field (or focus)
                # is left with a non-matching value
                entry.bind('<FocusOut>', lambda e, v=variable: self._validate_pattern_field(v))
            except Exception:
                pass

        # Instance-level override; chibiui's class definition is unchanged.
        ui._create_textbox = patched_create_textbox

    def _apply_pattern_color(self, variable, entry, text=None):
        """Set the textbox's text color based on whether it matches the pattern."""
        try:
            if text is None:
                text = entry.get()
            if self.patterns[variable].fullmatch(str(text)):
    ...
    def _on_pattern_text_changed(self, variable):
        """Called on every keystroke in a pattern-validated textbox to update its color live."""
        entry = self.pattern_entries.get(variable)
        entry_var = self.pattern_vars.get(variable)
        if entry is None or entry_var is None or not self.ui or not self.IsAlive:
            return
        self._apply_pattern_color(variable, entry, text=entry_var.get())

    def _validate_pattern_field(self, variable):
        """
        Validate a textbox's current UI value against its registered regex
        pattern. Called when the textbox loses focus (or focus moves to
        another widget). If the value doesn't fully match the pattern, the
        textbox is reverted to the last valid (current) value instead of
        being committed.

        Args:
            variable (str): The parameter path this textbox belongs to.
        """
        if not self.ui or not self.IsAlive or variable not in self.patterns:
            return
        path = self.widget_paths.get(variable)
        if path is None:
            return
        try:
            current_value = self.ui.get(path)
            if current_value is None:
                return
            if self.patterns[variable].fullmatch(str(current_value)):
                self.last_valid_value[variable] = current_value
            else:
                last_valid = self.last_valid_value.get(variable, self._get_nested_value(variable))
                self.ui.set(path, last_valid)
            entry = self.pattern_entries.get(variable)
            if entry is not None:
                self._apply_pattern_color(variable, entry, text=self.last_valid_value.get(variable))
        except Exception:
            pass

    def _minimize_nav_tree(self):
        """
        Minimize the navigation tree panel to the left edge. Used when the
        parameter hierarchy only has the Root level, since the tree isn't
        needed for navigation in that case.

        Implemented purely through the tkinter widget references already
        exposed by chibiui (self.ui.root, self.ui.nav_tree) - the chibiui
        class itself is not modified.
        """
        if not self.ui or self.ui.nogui or not self.ui.root:
            return

        def _do_minimize():
            try:
                nav_tree = self.ui.nav_tree
                nav_frame = nav_tree.master
                main_paned = nav_frame.master
                main_paned.paneconfigure(nav_frame, width=1)
                main_paned.sash_place(0, 1, 0)
            except Exception:
                pass

        self.ui.root.after(0, _do_minimize)
        ...
        for variable, path in self.widget_paths.items():
            try:
                ...
                if ui_value is None:
                    continue
                if variable in self.patterns:
                    if not self.patterns[variable].fullmatch(str(ui_value)):
                        # Value doesn't match the pattern.
                        # GUI mode: the FocusOut handler (see _setup_pattern_validation)
                        # already reverts the textbox when the user leaves the field,
                        # so just skip committing this value to Prm while it's invalid.
                        # Headless mode has no focus events, so revert immediately here.
                        if self.ui.nogui:
                            last_valid = self.last_valid_value.get(variable, self._get_nested_value(variable))
                            self.ui.set(path, last_valid)
                        continue
                    self.last_valid_value[variable] = ui_value
...
if __name__ == "__main__":
    ...
    ParameterTable = [
        ...
        ['Name1','Name 1','Taro','[A-Za-z ]{0,32}'] # pu.Prm.Name1 (letters/spaces only, up to 32 chars)
    ]
